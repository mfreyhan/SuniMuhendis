...
def build_case(stages=1,streamtubes=10):
    ...
    for i in range(stages):
        stage=f"stage_{i+1}"; rows += [{"row_id":f"rotor_{i+1}","stage_id":stage,"row_type":"rotor","axial_location_m":cax*(2*i+1),"blade_count":40,"axial_chord_m":cax,"metal_angle_in_deg":_span(0.0),"metal_angle_out_deg":_span(-23.87)},{"row_id":f"stator_{i+1}","stage_id":stage,"row_type":"stator","axial_location_m":cax*(2*i+2),"blade_count":40,"axial_chord_m":cax,"metal_angle_in_deg":_span(0.0),"metal_angle_out_deg":_span(40.0)}]
