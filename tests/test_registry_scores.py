...
from sunimuhendis.environments.heat_exchanger.score import (
    HeatExchangerScoreV1,
    HeatExchangerScoreV2,
    HeatExchangerScoreV3,
)
...
def test_make_env_v3():
    env = make_env("heat_exchanger", score_version="heat_exchanger_score_v3")
    assert isinstance(env, HeatExchangerEnv)
    assert isinstance(env.score_function, HeatExchangerScoreV3)
...
def test_dynamic_score_resolution():
    ...
    task_params = {"score_version": "heat_exchanger_score_v3"}
    score_fn = env.get_score_function(task_params)
    assert isinstance(score_fn, HeatExchangerScoreV3)
